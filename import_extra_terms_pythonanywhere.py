#!/usr/bin/env python3
"""
Script để import dữ liệu từ terms_extra.xlsx vào database Django trên PythonAnywhere
"""
...
# Thêm đường dẫn PythonAnywhere
sys.path.append('/home/timtruonghoc/timtruonghoc')
sys.path.append('/home/timtruonghoc/timtruonghoc/apptimtruonghoc')

# Thiết lập Django - thử các settings khác nhau
try:
    os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'settings')
    django.setup()
    from models import Term, Category
    print("✅ Sử dụng settings.py")
except Exception as e1:
    try:
        os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'apptimtruonghoc.settings')
        django.setup()
        from apptimtruonghoc.models import Term, Category
        print("✅ Sử dụng apptimtruonghoc.settings")
    except Exception as e2:
        try:
            os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'timtruonghoc.settings')
            django.setup()
            from timtruonghoc.models import Term, Category
            print("✅ Sử dụng timtruonghoc.settings")
        except Exception as e3:
            print("❌ Không thể thiết lập Django settings")
            print(f"Lỗi 1: {e1}")
            print(f"Lỗi 2: {e2}")
            print(f"Lỗi 3: {e3}")
            sys.exit(1)
...
def import_extra_terms():
    ...
    # Đường dẫn đến file Excel trên PythonAnywhere
    excel_paths = [
